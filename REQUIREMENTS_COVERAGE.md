...
| 5 | mTLS, per-device identity, on-device keygen→CSR→cert | **Partially open** | mTLS client path wired (PubSubClient over MbedTLSClient; TLS material from KVS; `mbedtls_net_set_nonblock` Beken gap shimmed). Keygen/CSR/cert exchange = backend contract (R-19, release-gated) |
...
| 7 | One-time enrollment token (R-19) | **Dev placeholder** | `KvsDevEnrollment` stores the token and gates re-provisioning; it does NOT fetch a cert (backend contract). Clearly labeled in code |
...
labeled placeholders: the enrollment backend (R-19), the production
signature verifier (R-23/Ed25519-vs-ECDSA decision), and the R-0 soak.
